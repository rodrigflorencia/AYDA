/**
 * @file Grafo.tpp
 * @brief Implementación de un grafo con listas enlazadas propias (sin STL para
 * nodos/aristas).
 *
 * Representación: cada vértice es un nodo enlazado (lista simple). Cada nodo
 * mantiene una lista enlazada de Arco que representan las adyacencias
 * salientes. Soporta grafos dirigidos y no dirigidos (controlado por @c
 * noDirigido).
 *
 * @tparam TipoVertice tipo de la etiqueta de cada vértice (debe tener operador
 * == y ser copiable).
 * @tparam TipoArco tipo del peso/costo del arco (debe ser copiable).
 *
 * @section ownership Propiedad de memoria
 * - @c getVertices y @c getAdyacentes devuelven arreglos @c new[] que deben
 * liberarse con @c delete[] por el llamador.
 *
 * @section complexity Complejidades globales
 * - Estructura por listas enlazadas: operaciones típicas dependen del grado o
 ...
 *
 * @param noDir Si es true, el grafo será no dirigido; si es false, será
 * dirigido.
 * @tparam TipoVertice
 * @tparam TipoArco
 * @complexity O(1)
...
/**
 * @brief Elimina el vértice con etiqueta @p vertice y todas sus aristas
 * incidentes.
 *
 * Quita aristas entrantes desde otros vértices y las salientes desde el propio
 * vértice.
 *
...
/**
 * @brief Agrega una arista @c o -> @c d con peso @p peso. En no dirigido,
 * agrega también @c d -> @c o si no existe.
 *
 ...
 * @return true si se agregó; false si no existen los vértices o la arista ya
 * existía.
 * @tparam TipoVertice
 * @tparam TipoArco
 * @complexity O(n) para encontrar el vertice origen entre los n vertices +
 * verificar duplicado (grado(o)) +  insertar cte
 * @note En grafo no dirigido suma dos aristas y actualiza contadores en ambos
 * sentidos.
 */
template <class TipoVertice, class TipoArco>
...
/**
 * @brief Elimina la arista @c o -> @c d. En no dirigido, elimina también @c d
 * -> @c o.
 *
 * @param o Vértice origen.
 * @param d Vértice destino.
 ...
 * @tparam TipoVertice
 * @tparam TipoArco
 * @complexity O(o+grado(o)) para localizar y eliminar; en no dirigido también +
 * O(d+grado(d)).
 */
template <class TipoVertice, class TipoArco>
 ...
 * @complexity O(n) porque n vertices y grado (n) para la arista
 *
 * @warning El puntero retornado es válido mientras no se modifique/elimine la
 * arista o el grafo.
 */
template <class TipoVertice, class TipoArco>
const TipoArco *
...
/**
 * @brief Actualiza el costo de la arista @c o -> @c d (y opcionalmente @c d ->
 * @c o).
 *
 * Si el grafo es no dirigido, intenta actualizar también la arista inversa si
 * existe.
 *
 * @param o Vértice origen.
 * @param d Vértice destino.
 ...
 * @tparam TipoVertice
 * @tparam TipoArco
 * @complexity O(n). n hasta encontrar el vertice, + grado (n) para encontrar la
 * arista
 ...
 *
 * Formato: por cada vértice, imprime sus aristas salientes como "u -> v
 * [peso]".
 *
 * @tparam TipoVertice
 * @tparam TipoArco
